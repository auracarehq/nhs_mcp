"""Router for NHS Medicines domain."""
...
router = APIRouter(prefix="/medicines", tags=[LABEL])


@router.post("/scrape", response_model=TaskResponse, summary=f"Scrape all {LABEL}")
async def scrape_all_medicines(force: bool = False) -> TaskResponse:
    """Start a background task that scrapes every item from the NHS Medicines A-Z index."""
...
@router.post("/scrape/{slug}", response_model=TaskResponse, summary="Scrape a single medicine")
async def scrape_one_medicine(slug: str) -> TaskResponse:
    """Start a background task that scrapes one medicine by slug."""
...
async def update_stale_medicines() -> TaskResponse:
    """Re-scrape all Medicines whose next_review_due date has passed."""
...
async def list_medicines() -> list[ItemSummary]:
    """List all previously scraped Medicines."""
    pool = db.get_pool()
    rows = await db.list_pages(pool, DOMAIN)
    return [ItemSummary(slug=r["slug"], name=r["name"]) for r in rows]


@router.get("/{slug}", response_model=ItemContent, summary="Get a scraped medicine")
async def get_medicine(slug: str) -> ItemContent:
    """Retrieve the full markdown content and metadata for a single medicine."""
    pool = db.get_pool()
...
@router.delete("/{slug}", summary="Delete a scraped medicine")
async def delete_medicine(slug: str) -> dict:
    """Remove a scraped medicine from the database."""
    pool = db.get_pool()
